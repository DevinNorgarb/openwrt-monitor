...
- With Telegraf collectd multivalue split, per-interface octets use measurement names `{plugin}_{dataSource}` with field `value` and tag `instance` (netdev). OpenWrt commonly emits **`interface_0`** / **`interface_1`** (if_octets DS names `0`/`1`); stock types.db often yields **`interface_rx`** / **`interface_tx`**.
